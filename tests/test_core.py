...
from fgpbot.health import Health, SingleInstance, atomic_json, read_status
from fgpbot.security import REDACT, Redactor, SafeFormatter
from fgpbot.storage import Store
from tests.helpers import StoreCase, notification, ready
...
class ConfigTests(unittest.TestCase):
    def setUp(self):
        ...
        self.env = {"TWITCH_BOT_APP_CLIENT_ID": "test-client-id", "TWITCH_BOT_APP_CLIENT_SECRET": "synthetic-secret",
                    "TWITCH_BOT_ID": "100", "TWITCH_OWNER_ID": "200"}

    def test_legacy_owner_id_is_channel_fallback(self):
    ...
    def test_channel_override_and_explicit_direct_connection(self):
    ...
    def test_config_is_anchored_to_root_not_working_directory(self):
        (self.root / ".env").write_text("\n".join(f"{k}={v}" for k,v in self.env.items()))
        self.assertEqual(load_config(self.root, {}).bot_id, "100")

    def test_environment_overrides_dotenv_without_mutating_environment(self):
    ...
    def test_rejects_invalid_inputs_without_echoing_secrets(self):
        for field, value in (("TWITCH_BOT_ID", "nickname"), ("TWITCH_CHANNEL_ID", "-1"),
                             ("FGP_PROXY_URL", "socks5://secret-user:secret-pass@host:10"),
                             ("FGP_QUEUE_SIZE", "0"), ("FGP_LOG_CHAT", "maybe"),
                             ("FGP_PREFIX", "two words"), ("TWITCH_REDIRECT_URI", "http://0.0.0.0:4343/oauth/callback")):
...
class PureCommandTests(unittest.TestCase):
    ...
    def test_russian_plural_rules(self):
        self.assertEqual([russian_word(n,"год","года","лет") for n in (1,2,5,11,21,22,112)],
                         ["год","года","лет","лет","год","года","лет"])

    def test_durations_are_not_rounded_to_next_unit(self):
        for seconds, expected in ((59*60, "59 минут"), (23*3600,"23 часа"),
                                  (364*86400,"364 дня"), (0,"0 секунд"), (-1,"0 секунд"),
    ...
    def test_queue_output_bounded_and_empty(self):
...
class SecurityTests(unittest.TestCase):
    def test_redacts_urls_headers_dictionaries_and_encoded_values(self):
        r = Redactor()
        r.add("secret with/slashes")
        for text in ("?refresh_token=abcdef123456&client_secret=zyx987654", "Bearer abcdef123456",
                     "{'access_token': 'abcdef123456'}", "http://user:password@localhost:12334",
                     "code=abcdef123456", "secret%20with%2Fslashes"):
    ...
    def test_redacts_exception_traceback_after_formatting(self):
        REDACT.add("synthetic-trace-secret")
        stream = io.StringIO()
        handler = logging.StreamHandler(stream)
        handler.setFormatter(SafeFormatter("%(message)s"))
        logger = logging.Logger("test-only")
    ...
    def test_redaction_preserves_valid_status_json(self):
        with tempfile.TemporaryDirectory() as folder:
            path = Path(folder) / "status.json"
            REDACT.add("synthetic-json-secret")
            atomic_json(path, {"schema":1, "written_at": time.time(), "last_error": "token=synthetic-json-secret", "status":"FAILED"})
            self.assertEqual(read_status(path)["status"], "FAILED")
            self.assertNotIn("synthetic-json-secret", path.read_text())


class HealthTests(unittest.TestCase):
    def test_http_ok_alone_never_means_chat_ready(self):
    ...
    def test_ready_does_not_claim_end_to_end_test(self):
    ...
    def test_stale_keepalive_or_worker_not_healthy(self):
        state = Health("100", "200")
        ready(state)
    ...
    def test_delivery_failure_probe_failure_and_overload_not_green(self):
        state = Health("100", "200")
        ready(state)
        for field, value, cleared in (("last_delivery_error", "rejected", ""), ("e2e","FAILED","UNVERIFIED"),
    ...
    def test_disk_status_missing_corrupt_stale_and_future(self):
        with tempfile.TemporaryDirectory() as folder:
            path=Path(folder)/"status.json"
            ...
            for date in (time.time()-30, time.time()+100):
                atomic_json(path, {"schema":1,"written_at":date,"status":"READY","transport_ready":True})
    ...
    def test_cli_requires_consent_before_any_probe_configuration_or_network(self):
...
class StorageTests(StoreCase):
    async def test_migration_preserves_legacy_tokens_and_messages(self):
        ...
        with closing(sqlite3.connect(old)) as db, db:
            db.execute("CREATE TABLE tokens(user_id TEXT PRIMARY KEY, token TEXT NOT NULL, refresh TEXT NOT NULL)")
            db.execute("INSERT INTO tokens VALUES('100','legacy-access','legacy-refresh')")
        store = Store(old)
        await store.initialize()
        self.assertEqual((await store.token("100"))["token"], "legacy-access")
        event = notification()["payload"]["event"]
        await store.log_message(event,"2020-01-01T00:00:00Z")
        await store.initialize()
        await store.cleanup(0)
        self.assertEqual(await store.call(lambda db: db.execute("SELECT count(*) FROM messages").fetchone()[0]),1)

    async def test_compare_and_swap_cannot_overwrite_a_new_authorization(self):
        await self.store.save_token("100", "original", "original-refresh")
        await self.store.save_token("100", "manual-new", "manual-new-refresh")
        saved = await self.store.save_token("100", "stale-rotation", "stale-rotation-refresh", expected=("original","original-refresh"))
        self.assertFalse(saved)
        self.assertEqual((await self.store.token("100"))["token"],"manual-new")

    async def test_message_insert_is_idempotent_and_follower_unknown(self):
        event = notification()["payload"]["event"]
    ...
    async def test_claims_are_atomic_across_simultaneous_workers_and_restart(self):
    ...
    async def test_probe_confirmed_only_for_matching_message_id(self):
        nonce="a"*32
    ...
    async def test_probe_echo_can_arrive_before_post_response(self):
        nonce="b"*32
        await self.store.new_probe(nonce,"run")
        await self.store.claim_probe("run")
        await self.store.probe_observed(nonce,"actual","run")
        self.assertEqual((await self.store.probe(nonce))["state"],"SENDING")
        await self.store.probe_sent(nonce,"actual")
        self.assertEqual((await self.store.probe(nonce))["state"],"CONFIRMED")

    async def test_probe_forgery_wrong_run_unknown_nonce_and_throttle(self):
        nonce="c"*32
    ...
    async def test_restart_expires_pending_probe_instead_of_replaying(self):
        await self.store.new_probe("c"*32,"old-run")
        self.assertIsNone(await self.store.claim_probe("new-run"))
        self.assertEqual((await self.store.probe("c"*32))["state"],"FAILED")

    async def test_doctor_offline_missing_token_explicit(self):
        report = await cli.doctor(self.config, True)
...
class AtomicWriteTests(StoreCase):
    async def test_concurrent_json_writes_leave_complete_file_and_no_temporary_files(self):
        await asyncio.gather(*(asyncio.to_thread(atomic_json,self.config.status_file,
            {"schema":1,"written_at":time.time(),"status":"READY","run_id":str(i)}) for i in range(8)))
        self.assertEqual(read_status(self.config.status_file)["status"],"READY")
        self.assertEqual(list(self.config.data_dir.glob(".*.tmp")),[])

    async def test_cancelled_status_writer_finishes_inflight_write_before_shutdown(self):
        import threading
        from fgpbot.health import status_writer
    ...
    async def test_bad_nonfinite_timestamp_is_not_a_fresh_ready_status(self):
