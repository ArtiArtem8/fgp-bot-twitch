...
class LocalTwitch:
    ...
    async def start(self):
        ...
        await site.start()
        # Private test-server inspection only, never production networking patches.
    ...
    async def validate(self, request):
        ...
        if (access == "synthetic-bot-access" and self.bot_valid) or access == "synthetic-rotated-access":
    ...
    async def refresh(self, request):
        ...
        if form.get("grant_type") == "authorization_code":
            ...
            return web.json_response({"access_token":"synthetic-bot-access","refresh_token":"synthetic-bot-refresh"})
        ...
        self.rotated=True
        return web.json_response({"access_token":"synthetic-rotated-access", "refresh_token":"synthetic-rotated-refresh"})

    async def users(self, request):
        ids=request.query.getall("id",[])
        return web.json_response({"data":[{"id":i,"login":"streamer" if i=="200" else "fgp_test_bot"} for i in ids]})

    async def subscriptions(self, request):
        assert request.headers.get("Authorization") in {"Bearer synthetic-bot-access","Bearer synthetic-rotated-access"}
    ...
    async def send(self, request):
        body=await request.json()
        assert body["broadcaster_id"]=="200" and body["sender_id"]=="100"
        assert "for_source_only" not in body, "Invalid with user token"
        self.sent.append(body)
        message_id=f"sent-{len(self.sent)}"
        if self.drop_send:
            return web.json_response({"data":[{"message_id":"","is_sent":False,"drop_reason":{"code":"automod_held","message":"held"}}]})
        if self.echo:
            for sub in list(self.subs):
                ...
                if ws and not ws.closed:
                    await ws.send_json(notification(body["message"],message_id=message_id,user_id="100"))
        if self.echo_before_response:
            await asyncio.sleep(.1)
        return web.json_response({"data":[{"message_id":message_id,"is_sent":True,"drop_reason":None}]})
...
class IntegrationTests(StoreCase):
    ...
    async def test_original_incident_revoked_owner_but_bot_chat_works(self):
    ...
    async def test_startup_refresh_persists_rotated_pair_and_receives_command(self):
        self.server.bot_valid=False
        await self.start_bot()
        self.assertEqual(self.server.refresh_count,1)
        self.assertEqual((await self.store.token("100"))["refresh"],"synthetic-rotated-refresh")
        await self.server.emit(notification("!ping"))
        await until(lambda:self.state.sent==1)

    async def test_http_200_drop_is_not_reported_as_sent(self):
    ...
    async def test_disconnect_creates_new_subscription_and_commands_resume(self):
        await self.start_bot()
        first=self.state.session_id
        await self.server.sockets[first].close()
        await until(lambda:self.state.phase=="LISTENING" and self.state.session_id!=first,timeout=8)
    ...
    async def test_real_websocket_handoff_does_not_recreate_subscriptions(self):
        await self.start_bot()
        first=self.state.session_id
        ...
        await self.server.emit({"metadata":{"message_type":"session_reconnect"},"payload":{"session":{"reconnect_url":"wss://eventsub.wss.twitch.tv/ws?test"}}},first)
    ...
    async def test_full_probe_cli_to_api_to_websocket_to_command_handler(self):
    ...
    async def test_probe_cannot_pass_when_twitch_accepts_send_but_no_echo_arrives(self):
        ...
        await self.store.call(lambda db:db.execute("UPDATE fgp_probes SET created=? WHERE nonce=?",(time.time()-36,nonce)))
        await until(lambda:self.state.e2e=="FAILED",timeout=3)
        self.assertEqual(self.state.snapshot()["status"],"DEGRADED")
        self.assertEqual(len(self.server.sent),1)

    async def test_doctor_checks_current_subscription_without_refresh_or_messages(self):
        await self.start_bot()
        atomic_json(self.config.status_file,self.state.snapshot())
        report=await cli.doctor(self.config,False)
    ...
    async def test_cancellation_closes_websocket_and_no_workers_survive(self):
        task=await self.start_bot()
        await cancel(task)
        ...
        self.assertTrue(names.isdisjoint({"commands","audit","probes","status-writer","eventsub"}),names)

    async def test_http_chunked_json_is_read_to_eof(self):
        result=await self.http.request("GET",self.server.base+"/http/chunked")
        self.assertEqual(result,{"value":42})

    async def test_http_invalid_and_oversized_responses_fail_explicitly(self):
        for mode in ("invalid","large"):
            with self.assertRaises(ProtocolError):
                await self.http.request("GET",self.server.base+"/http/"+mode)

    async def test_http_safe_get_retries_503_once(self):
        self.assertEqual(await self.http.request("GET",self.server.base+"/http/retry"),{"ok":True})
        self.assertEqual(self.server.http_counts["retry"],2)

    async def test_http_4xx_not_retried_and_redirect_not_followed(self):
    ...
    async def test_http_post_connection_lost_after_receipt_never_replayed(self):
        with self.assertRaises(NetworkError):
            await self.http.request("POST",self.server.base+"/http/ambiguous",json={"message":"test"})
        self.assertEqual(self.server.http_counts["ambiguous"],1)

    async def test_lost_remote_subscription_detected_and_repaired_while_api_stays_up(self):
        self.patches.enter_context(patch("fgpbot.app.AUDIT_INITIAL_DELAY",.05))
        self.patches.enter_context(patch("fgpbot.app.AUDIT_INTERVAL",.05))
        await self.start_bot()
        first=self.state.session_id
    ...
    async def test_complete_local_oauth_listener_callback_exchange_and_shutdown(self):
        import socket
        ...
        self.patches.enter_context(patch("fgpbot.auth.TOKEN_URL",self.server.base+"/oauth2/token"))
        ...
        def fake_browser(url):
            ...
            return True
        with patch("fgpbot.auth.webbrowser.open",fake_browser),patch("sys.stdout",io.StringIO()):
            task=asyncio.create_task(authorize(conf,"bot",False,True))
            try:
                ...
                await cancel(task)
        self.assertEqual((await self.store.token("100"))["refresh"],"synthetic-bot-refresh")
        self.assertEqual(len(self.server.sent),0)
