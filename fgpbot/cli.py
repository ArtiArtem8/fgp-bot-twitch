...
from .config import CHAT_SCOPES, ROOT, Config, ConfigError, load_config
...
def show_status(data: dict, as_json: bool = False) -> None:
    if as_json:
        print(json.dumps(data, ensure_ascii=False, indent=2))
        return
    print(f"FGPbot: {data.get('status', 'UNKNOWN')}")
    if data.get("status") in {"STALE", "NOT_RUNNING", "UNKNOWN"}:
        print("Нет свежего подтверждения от процесса. Старые успешные проверки не означают, что бот сейчас работает.")
    ...
    print(f"Сообщений: {data.get('received', 0)} | команд: {data.get('commands', 0)} | отправлено API: {data.get('sent', 0)}")
    print(f"Ошибки команд: {data.get('command_errors', 0)} | отправки: {data.get('send_errors', 0)} | потери очереди: {data.get('dropped_events', 0)}")
    print(f"Сквозная проверка: {data.get('e2e', 'UNVERIFIED')}")
    for key, label in (("last_message_at", "Последнее сообщение"), ("last_send_at", "Последняя отправка"),
                       ("last_probe_at", "Время сквозной проверки")):
        if data.get(key) is not None:
            try:
                stamp = datetime.fromtimestamp(float(data[key])).astimezone().strftime("%Y-%m-%d %H:%M:%S %Z")
                print(f"{label}: {stamp}")
            except (ValueError, OverflowError, OSError, TypeError):
                print(f"{label}: неизвестно")
    if data.get("last_probe_detail"):
        print("Последняя проверка: " + data["last_probe_detail"])
    for key, label in (("last_error", "Последняя ошибка"), ("last_delivery_error", "Ошибка отправки")):
        if data.get(key):
...
async def doctor(config: Config, offline: bool) -> dict:
    report = {
        "python": platform.python_version(), "platform": platform.system(),
        "aiohttp": importlib.metadata.version("aiohttp"),
        "configuration": "OK", "bot_id": config.bot_id, "channel_id": config.channel_id,
        "channel_source": "TWITCH_CHANNEL_ID, либо совместимый fallback TWITCH_OWNER_ID",
        "proxy": REDACT(config.proxy or "DIRECT"), "database_exists": config.database.exists(),
        "runtime": read_status(config.status_file), "sends_chat_messages": False,
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        ...
        try:
            identity = await tokens.validate(bot["token"], config.bot_id)
            ...
            report["bot_authorization"] = "VALID" if not report["missing_chat_scopes"] else "MISSING_SCOPES: запусти auth"
            headers = {"Client-Id": config.client_id, "Authorization": f"Bearer {bot['token']}"}
            users = await http.request("GET", HELIX + "/users", params={"id": config.channel_id}, headers=headers)
            report["channel"] = [{"id": u["id"], "login": u["login"]} for u in users.get("data", [])]
            subs, params, cursors = [], {}, set()
            for _ in range(100):
                response = await http.request("GET", HELIX + "/eventsub/subscriptions", params=params, headers=headers)
            ...
            report["note"] = "Doctor не обновляет токены. Истёкший access token ещё может быть восстановлен ботом через refresh."
    return report
...
async def check_chat(config: Config) -> int:
    ...
    print("Запрошено ОДНО диагностическое сообщение в целевом чате; проверяется его обратная доставка.")
    ...
    while time.monotonic() < deadline:
        ...
        if current.get("status") in {"STALE", "NOT_RUNNING", "STOPPED", "FAILED"} or current.get("run_id") != runtime["run_id"]:
    ...
    print("FAIL: нет подтверждения за отведённое время. Автоматический повтор отправки НЕ выполняется.")
    return 1
...
def parser() -> argparse.ArgumentParser:
    ...
    auth.add_argument("--followers", action="store_true", help="Запросить bot scope moderator:read:followers")
    ...
    check.add_argument("--send", action="store_true", help="Разрешить одно публичное диагностическое сообщение")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == "status":
        ...
        show_status(status, arguments.json)
        return 0 if status.get("status") == "READY" else 1
    if command == "check-chat" and not arguments.send:
        print("Нужен явный --send: проверка публикует одно сообщение в чате. Для тихой проверки используй status или doctor.")
        return 2
    try:
        config = load_config()
        REDACT.add(config.client_secret, config.music_token)
        # Only the run process writes the rotating log; auth/doctor cannot race its rotation on Windows.
        if command == "run":
            try:
            ...
            return 0
        if command == "auth":
            return 0 if asyncio.run(authorize(config, arguments.account, arguments.followers, not arguments.no_browser)) else 2
        if command == "doctor":
            report = asyncio.run(doctor(config, arguments.offline))
            print(REDACT(json.dumps(report, ensure_ascii=False, indent=2)))
            failed = (report.get("online_error") or report.get("database_error")
                      or not report.get("bot_token_stored") or report.get("missing_chat_scopes"))
            if not arguments.offline:
                failed = (failed or not report.get("channel") or not report.get("chat_subscriptions_on_current_session")
