...
class Store:
    ...
    async def call(self, callback: Callable[[sqlite3.Connection], Any]) -> Any:
        return await asyncio.to_thread(self._transaction, callback)
    ...
    async def token(self, user_id: str) -> dict | None:
        row = await self.call(lambda db: db.execute(
            "SELECT user_id, token, refresh FROM tokens WHERE user_id=?", (user_id,)
        ).fetchone())
        return dict(row) if row else None

    async def save_token(self, user_id: str, token: str, refresh: str,
                         expected: tuple[str, str] | None = None) -> bool:
        def save(db: sqlite3.Connection) -> bool:
            if expected is not None:
                cur = db.execute(
                    "UPDATE tokens SET token=?, refresh=? WHERE user_id=? AND token=? AND refresh=?",
                    (token, refresh, user_id, *expected),
                )
            else:
                cur = db.execute(
                    "INSERT INTO tokens(user_id,token,refresh) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE "
                    "SET token=excluded.token, refresh=excluded.refresh", (user_id, token, refresh),
                )
            return cur.rowcount == 1
        return await self.call(save)

    async def claim_event(self, key: str) -> bool:
        return await self.call(lambda db: db.execute(
            "INSERT OR IGNORE INTO fgp_seen VALUES(?,?)", (key, time.time())
        ).rowcount == 1)

    async def log_message(self, event: dict, timestamp: str) -> None:
        badges = event.get("badges", [])
        subscriber = any(b.get("set_id", "").lower() in {"subscriber", "founder"} for b in badges)
        values = (
            event["message_id"], event["chatter_user_id"], event.get("chatter_user_login", ""),
            event.get("chatter_user_name", ""), event["broadcaster_user_id"],
            event["message"]["text"], timestamp, json.dumps(badges, ensure_ascii=False),
            subscriber, None, event.get("message_type", "text"),
        )
        # Follower status is unknown, NOT false. No API request per message.
        await self.call(lambda db: db.execute(
            "INSERT OR IGNORE INTO messages(message_id,user_id,username,display_name,channel_id,"
    ...
    async def new_probe(self, nonce: str, run_id: str) -> None:
        def create(db: sqlite3.Connection) -> None:
            ...
            latest = db.execute("SELECT created FROM fgp_probes ORDER BY created DESC LIMIT 1").fetchone()
            if latest and time.time() - latest[0] < 30:
                raise ValueError("Проверка уже запрошена недавно. Интервал — не менее 30 секунд.")
            db.execute("INSERT INTO fgp_probes(nonce,created,run_id,state) VALUES(?,?,?,'REQUESTED')",
                       (nonce, time.time(), run_id))
        await self.call(create)

    async def claim_probe(self, run_id: str) -> dict | None:
        def claim(db: sqlite3.Connection) -> dict | None:
            db.execute("BEGIN IMMEDIATE")
            db.execute("UPDATE fgp_probes SET state='FAILED',detail='Бот перезапущен или запрос устарел' "
                       "WHERE state IN ('REQUESTED','SENDING','SENT') AND (run_id!=? OR created<?)",
                       (run_id, time.time() - 45))
            row = db.execute("SELECT * FROM fgp_probes WHERE state='REQUESTED' AND run_id=? "
                             "ORDER BY created LIMIT 1", (run_id,)).fetchone()
            if not row:
                return None
    ...
    async def probe(self, nonce: str) -> dict | None:
        row = await self.call(lambda db: db.execute("SELECT * FROM fgp_probes WHERE nonce=?", (nonce,)).fetchone())
        return dict(row) if row else None

    async def probe_sent(self, nonce: str, message_id: str) -> None:
        await self.call(lambda db: db.execute(
            "UPDATE fgp_probes SET message_id=?,state=CASE WHEN observed_id=? THEN 'CONFIRMED' ELSE 'SENT' END "
            "WHERE nonce=? AND state='SENDING'", (message_id, message_id, nonce)
        ).rowcount)

    async def probe_observed(self, nonce: str, message_id: str, run_id: str) -> bool:
        return await self.call(lambda db: db.execute(
            "UPDATE fgp_probes SET observed_id=?,state=CASE WHEN message_id=? THEN 'CONFIRMED' ELSE state END "
    ...
    async def probe_failed(self, nonce: str, detail: str) -> None:
        await self.call(lambda db: db.execute(
            "UPDATE fgp_probes SET state='FAILED',detail=? WHERE nonce=? AND state!='CONFIRMED'", (detail, nonce)
        ).rowcount)
    ...
    async def cleanup(self, chat_retention_days: int) -> None:
        def cleanup(db: sqlite3.Connection) -> None:
            ...
            if chat_retention_days > 0:
                db.execute("DELETE FROM messages WHERE julianday(timestamp)<julianday('now',?)",
