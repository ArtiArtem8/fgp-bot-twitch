...
from .config import CHAT_SCOPES, FOLLOW_SCOPE, Config
from .network import Http, ProtocolError
from .security import REDACT
from .storage import Store
from .tokens import TOKEN_URL, AuthRequired, Tokens


class Authorization:
    """Single-use, state-checked OAuth callback, bound only to loopback."""

    def __init__(self, config: Config, store: Store, http: Http,
                 account: str = "bot", followers: bool = False) -> None:
        self.config, self.store, self.http = config, store, http
        self.expected_user = config.bot_id if account == "bot" else config.channel_id
        self.scopes = CHAT_SCOPES | (FOLLOW_SCOPE if followers else frozenset()) if account == "bot" else FOLLOW_SCOPE
    ...
    def url(self) -> str:
        return "https://id.twitch.tv/oauth2/authorize?" + urlencode({
            ...
            "state": self.state, "force_verify": "true",
        })
    ...
    async def callback(self, request: web.Request) -> web.Response:
        ...
        if not secrets.compare_digest(supplied.encode("utf-8"), self.state.encode("utf-8")):
            return web.Response(status=400, text="OAuth state не совпадает. Откройте исходную ссылку авторизации.")
        if self.used:
        ...
        REDACT.add(code)
        try:
            response = await self.http.request("POST", TOKEN_URL, data={
                "client_id": self.config.client_id, "client_secret": self.config.client_secret,
                "code": code, "grant_type": "authorization_code", "redirect_uri": self.config.redirect_uri,
            })
            ...
            identity = await Tokens(self.config, self.store, self.http).validate(access, self.expected_user)
            missing = self.scopes - frozenset(identity["scopes"])
            if missing:
                raise AuthRequired("Не выданы запрошенные scopes: " + ", ".join(sorted(missing)))
            await self.store.save_token(self.expected_user, access, refresh)
            self.success = True
            self.result = f"Авторизован {identity.get('login', '')} (ID {self.expected_user}). Токены сохранены локально."
        except Exception as exc:
            self.result = REDACT(exc)
        self.done.set()
        return web.Response(status=200 if self.success else 400, content_type="text/html", text=(
            "<!doctype html><meta charset='utf-8'><title>FGPbot OAuth</title>"
            f"<h1>{'Готово' if self.success else 'Авторизация не завершена'}</h1><p>{html.escape(self.result)}</p>"
...
async def authorize(config: Config, account: str, followers: bool, open_browser: bool) -> bool:
    ...
    async with aiohttp.ClientSession(trust_env=False) as session:
        flow = Authorization(config, store, Http(session, config.proxy), account, followers)
        app = web.Application()
        ...
        try:
            ...
            print(f"В Twitch войди в {'аккаунт БОТА' if account == 'bot' else 'аккаунт СТРИМЕРА'}: ожидается ID {flow.expected_user}.")
